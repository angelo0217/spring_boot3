...
public class StockConst {
    ...
    @Getter
    public enum REASON {
      NOTHING("nothing"), MAGNIFICATION("交易量倍率達到" + StockConst.MAGNIFICATION + "倍"), RISE(
          "漲幅超過" + StockConst.RISE_LINE + "百分比");
      ...
      public static REASON getStockReason(Integer detectVolumes, Integer lastVolumes,
          Double detectMoney, Double lastMoney) {
        if ((detectVolumes / lastVolumes) > StockConst.MAGNIFICATION && detectMoney > lastMoney)
          return REASON.MAGNIFICATION;
        else if (((detectMoney - lastMoney) / lastMoney) * 100 > StockConst.RISE_LINE
            && detectVolumes > StockConst.DEFAULT_VOLUMES) {
          log.info("match {}, 倍率: {}", StockConst.RISE_LINE,
